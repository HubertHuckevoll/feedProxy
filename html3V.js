...
export class html3V
{
   ...
   */
  drawOverview(meta, feeds)
  {
    let erg = '';
    erg += this.openPage();
    erg += '<img src="'+this.imageProxy(meta.image, 128)+'"><br>';
    erg += '<h1>'+this.Utf8ToHTML(meta.title)+'</h1>';
    erg += '<p>'+this.Utf8ToHTML(meta.description)+'</p>';
    erg += '<ul>';
    for (const feed of feeds)
    {
      const feedHttp = feed.replace('https', 'http');
  ...
  drawArticlesForFeed(articles)
  {
    ...
    */
    ...
    if (articles.entries.length > 0)
    {
      ...
      {
        erg += '<p>';
        erg += '<a href="'+article.id.replace('https', 'http')+'">'+this.Utf8ToHTML(article.title.value)+'</a>';
        erg += '</p>';
